...
export function buzzToolDefinitions({ api, refresh, notify }) {
  return [
    ...
    },
    {
      ...
      execute: async ({ space_id, content, request_id }, { signal } = {}) => {
        const result = await api(`/api/spaces/${encodeURIComponent(space_id)}/messages`, {
          method: "POST",
          body: { content, source: "webmcp", idempotency_key: request_id || crypto.randomUUID() },
          signal,
        });
